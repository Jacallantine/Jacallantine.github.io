...
export default function Home() {
  ...
  const tapHeart = () => {
    if (heartBreaking) return;
    const nextTap = heartTaps + 1;
    setHeartTaps(nextTap);
    if (nextTap >= 3) {
      setHeartBreaking(true);
      window.setTimeout(() => setHeartOpened(true), 1250);
    }
  };
  ...
  const acceptDate = async () => {
    ...
    try {
      const notificationBody = [
        ...
        "",
        "Sent from the date invitation page on your portfolio.",
      ]
        .filter(Boolean)
        .join("\n");
      ...
      <section className="mobile-app" aria-label="Date invitation app">
        ...
        <AnimatePresence>
          {!heartOpened && (
            ...
            >
              <p className="eyebrow">Something is waiting for you</p>
              <h1>Tap the heart<br />to <em>break the seal.</em></h1>
              <div className="break-heart-wrap">
                {heartBreaking && (
                  <div className="heart-particles" aria-hidden="true">
                    {Array.from({ length: 34 }, (_, item) => (
                      <motion.i
                        key={item}
                        ...
                        animate={{
                          ...
                          y: ((item * 137) % 760) - 380,
                          opacity: [0, 1, 0.95, 0],
                          scale: [0, 0.7 + (item % 5) * 0.22, 1.25 + (item % 3) * 0.18],
                          rotate: item % 2 ? 160 + item * 17 : -150 - item * 13,
                        }}
                        transition={{
                          duration: 0.95 + (item % 4) * 0.08,
                          delay: (item % 8) * 0.025,
                          ease: [0.16, 1, 0.3, 1],
                        }}
                      >♥</motion.i>
                ...
                <motion.button
                  key={heartTaps}
                  type="button"
                  className={`break-heart taps-${Math.min(heartTaps, 3)}`}
                  onClick={tapHeart}
                  aria-label={`Tap the heart to open the invitation. ${Math.max(3 - heartTaps, 0)} taps remaining.`}
                  animate={
                    heartBreaking
                      ? { scale: [1, 1.22, 0.08], rotate: [0, -12, 18], opacity: [1, 1, 0] }
                      : { x: [0, -7, 7, -5, 5, 0], rotate: [0, -4, 4, -3, 3, 0], scale: [1, 1.05, 1] }
                  }
                  transition={{ duration: heartBreaking ? 0.62 : 0.42, ease: "easeInOut" }}
                  whileTap={{ scale: 0.9 }}
                >
                  <Heart size={122} strokeWidth={1.45} fill="currentColor" />
                  <span className="heart-shine" aria-hidden="true" />
                  <span className="heart-crack crack-one" aria-hidden="true" />
                  <span className="heart-crack crack-two" aria-hidden="true" />
                </motion.button>
              </div>
              <p className="tap-instruction">
                {heartBreaking ? "You found my invitation…" : heartTaps ? "Again — it’s starting to crack!" : "Give it a tap. Don’t be shy."}
              </p>
              <div className="tap-dots" aria-hidden="true">
                {[0, 1, 2].map((item) => <i key={item} className={item < heartTaps ? "filled" : ""} />)}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
